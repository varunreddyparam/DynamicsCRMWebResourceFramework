...
if (window.parent.Notify !== undefined)
    window.Notify = window.parent.Notify;

const xrmContext = window.Xrm !== undefined ? window.Xrm : window.parent.Xrm;
...
if (window.Notify === undefined) {
    ...
    Notify._prefix = "sva_"; // Change this if you have your own solution prefix (as long as the file structure's the same)
    ...
    Notify.add = function (message, level, uniqueId, buttons, durationSeconds) {
        if (!Notify._initialised) {
            ...
            if ($header.length > 0) {
                ...
                var baseUrl = xrmContext.Page.context.getClientUrl();
                $("<link/>", { rel: "stylesheet", href: baseUrl + "/WebResources/" + Notify._prefix + "/js/_sdk/libraries/notifyjs/notify.css" }).appendTo('head');
    ...
    Notify.remove = function (uniqueId, animation = true) {
